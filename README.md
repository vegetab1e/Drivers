...
Также **настоятельно рекомендую** использовать макросы `NEW_FEATURES_TESTING` и `USE_FLT_INSTEAD_ZW`. С первым будет меньше нагрузка на фильтр, но есть возможность пропустить запретное действо, а без последнего драйвер может (в теории, я не поймал) виснуть при выгрузке, так как Zw-функции пропускают запрос через весь стек и есть возможность что-то потерять, например, где-то не закрыть хэндл или счётчик референсов не уменьшить, а Flt-функции спускают его ниже по стеку и самому себе он прийти не должен, как-то так.
...
***Вообще лучше использовать мой `FileBlockerDriver.vcxproj` или собранный мной драйвер, не забыв правильно установить мой сертификат, и всё будет как надо.***
...
Также в этом репозитории есть проект приложения для управления фильтром из консоли, но он практически пустой (при [его](FilterControl/control/FilterControl.exe) запуске на целевой машине не забываем про [редисты](FilterControl/control/vc_redist_2015-2022_14.42.34438.0_x64.exe)).
