...
Также **настоятельно рекомендую** использовать макросы `NEW_FEATURES_TESTING` и `USE_FLT_INSTEAD_ZW`. С первым будет меньше нагрузка на фильтр, но есть возможность аппрувнуть запретное действо, а без последнего драйвер может (в теории, я не поймал) вешаться при выгрузке, так как Zw-функции прогоняют запрос через весь стек и есть возможность что-то потерять, например, где-то не закрыть хэндл или счётчик референсов не уменьшить, а Flt-функции спускают его ниже по стеку и самому себе он прилететь не должен, как-то так.
...
- когда в фильтр приходит `IRP_MJ_SET_INFORMATION`, то рассматриваются только `FileDispositionInformation` и `FileRenameInformation`, а значит файл будет открыт;
- когда в фильтр приходит `IRP_MJ_CREATE`, то рассматривается только `FILE_OPEN` с множеством ограничений по флагам и файл будет закрыт.

Для последней ситуации используется функция `isTextBlocked()`, которая является обёрткой для `isTextBlocked2()`.
